...
当前代码落地了 0.4.0 的稳定性基础、0.5.0 的个体结果闭环、0.5.1 的响应/预算修复、0.5.2 的多支柱崩塌/换挡修复和 0.6.0 的生产/转化诊断：
...
- 生产与转化日志保存产能、区域槽、商人容量、改良覆盖、当前生产分配、金币/战略资源储备和升级积压；这些信号暂不进入评分或驱动策略。
- SQL 在加载前清理本 Mod 的旧策略记录，防止版本升级后权重残留叠加。

尚未实现：世界分布、完整自身趋势、由经济诊断驱动的瓶颈控制、滚动策略组合、玩家模型、完整战争目标与路线、城市专精、外交/交易/间谍、完整胜利里程碑及静态难度配置拆分。0.6.0 没有固定或锁定胜利路线；游戏原有胜利策略仍可随局势变化。

## 7. 后续实现顺序

1. 用新日志复测固定种子，确认 0.5.2 状态机不回归，并验证生产、区域、资源和升级传感器的玩法脚本可用性。
