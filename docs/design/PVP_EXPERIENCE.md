...
| --- | ---: | --- |
| 远古/古典 | 80%-125% | 保留开局、地形和文明强势期差异 |
| 中世纪/文艺复兴 | 85%-120% | 防止中期滚雪球过早结束比赛 |
| 工业时代以后 | 90%-115% | 后期需要稳定存在可信胜利威胁 |
...
- 所有主要 AI 都接受监测，但未来的高强度恢复与胜利转化预算应优先分配给 1 至 3 个仍有基础的可信竞争者。

## 4. 自主战略系统

### 4.1 长期计划

每个 AI 应保存一个可持续多个评估周期的计划，而不是只保存“当前偏好”：

- 主要胜利方向和备选方向。
- 主要竞争对手、近期目标与预期收益。
- 经济、扩张、军队、升级和胜利项目的预算比例。
- `准备 -> 投入 -> 评估 -> 继续/撤退` 阶段。
- 计划置信度、最短承诺期和允许改换方向的条件。

只有地图变化、战争结果、玩家威胁或胜利里程碑足以推翻原假设时，AI 才应换计划。
...
城邦战争不得触发全帝国常备军、移民和奇观惩罚。主要文明战争只有在后续版本具备目标与可达性判断后，才能进一步区分全面进攻和局部防御。
...
- 早期/中期/后期三段竞争区间。
- 综合护栏权重下调；科技、文化、帝国同时只允许一个恢复焦点。
- 领先状态删除负科技、文化、生产、扩张和奇观权重。
- 主文明战争和城邦/其他次要战争分开；只有主文明战争触发完整动员。
- 奇观惩罚只保留一个主文明战争来源，并从 `-75` 降至 `-35`。
- 修正艺术博物馆和考古博物馆内部类型。
- 评估与指标使用同一回合；日志同时保存原始、受控和平滑比值。
- SQL 在加载前清理本 Mod 的旧策略记录，防止版本升级后权重残留叠加。

尚未实现：竞争者名额、世界分布、自身趋势、长期计划、玩家模型、战争目标与可达性、城市专精、外交/交易/间谍、完整胜利里程碑及静态难度配置拆分。文档描述这些内容是后续设计合同，不代表当前版本已经具备。

## 7. 后续实现顺序

1. 用新日志复测同一对局与固定种子，确认状态稳定、文化建筑生效、城邦战争不再误动员。
2. 加入世界中位数/上四分位、自身趋势和 1 至 3 个竞争者预算。
3. 建立长期计划、预算和策略冲突矩阵。
