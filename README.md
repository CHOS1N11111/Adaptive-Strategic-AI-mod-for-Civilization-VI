# Adaptive Strategic AI 0.2

这不是“给 AI 偷资源”的作弊包，而是一个可审计、可测量的 AI 工程基线。当前只支持 Gathering Storm，主要针对远古时代开局、标准速度和神级难度调校；建议新开游戏测试。0.2 加入了以玩家为参照的小幅节奏调整，目的是减少 AI 过早滚雪球或中后期完全掉队。
...
- 综合实力由科技数、市政数、科研与文化速率、城市数、人口和军力构成。每个分项都限制在 55%-145%，避免单一指标支配结果。
...
- 该机制不会直接增加或扣除任何产出、资源、科技进度、单位或战斗力。启用开关、检查周期和进出阈值位于 `AI/10_CoreEconomy.sql`，可直接调整。
