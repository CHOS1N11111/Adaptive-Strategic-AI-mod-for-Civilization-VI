...
- Catch-up support has a modest tier and a bounded severe tier. Severe support can activate when overall strength collapses or at least two core pillars fail together. On Deity only, confirmed severe support now also activates a capped `+20%` Production and `+15%` Science/Culture city-yield result layer; the exact inverse is applied after recovery. Only one pillar recovery runs at a time; an ineffective direction exits into its own cooldown so another valid pillar can take over.
...
While confirmed severe support is active on Deity, add `+20` percentage points to Production and `+15` to Science/Culture in the applicable row. Gold, Faith, combat strength, and experience are unchanged. Recovery removes the complete adaptive addition.
