# Adaptive Strategic AI 0.5.0

这不是“给 AI 偷资源”的作弊包，而是一个可审计、可测量的 AI 工程基线。当前只支持 Gathering Storm，建议远古时代开局并新建游戏测试。0.5.0 让所有存活主要 AI 独立接受竞争护栏调节，并根据恢复结果退出无效方向；它不设置竞争者名额，也不把 AI 锁死在固定胜利路线。
...
- 综合扶持分为轻度和强度受限的严重落后两档；支柱恢复同时最多一个，并在效果窗口结束后检查真实趋势，无效方向会退出并单独冷却。
- 战争记录区分正式宣战与真实接战；只有与主要文明前线接近或近期发生可归属战斗时，才启用本 Mod 的完整战争动员。
- 基建恢复同时考虑城市数、人口和领土面积，避免高人口或沿海文明长期误判为缺改良。
- 建造者、商人和移民预算会计入城市队列中正在生产及已排队的单位，避免多个城市重复响应同一缺口；额外扩张恢复只在帝国焦点且没有活跃主要战争时启用。
...
- 焦点运行 20 个标准等效回合后复查效果：平滑比值改善不足 3 个百分点且原始比值改善不足 1 个百分点时，判定无效并让该支柱冷却 16 回合；其他支柱仍可正常参与。
- 综合比值低于 75% 时可进入强扶持档，高于 82% 时退出；两档都只增加有上限的原生 AI 决策权重，不直接赠送产出、单位或进度。
...
基建恢复的目标改良数为：`max(2 × 城市数, min(0.65 × 人口, 0.30 × 已拥有地块))`。已有及在产建造者会按每个 2 个潜在改良抵扣目标；在产商人同样抵扣商路缺口。扩张恢复允许的已有及在产移民上限为 `max(1, ceil(城市数 / 8))`。
...
`AI/10_CoreEconomy.sql` 中的 `ASAI_ENABLE_METRICS` 在当前测试版默认为 `1`。每次控制器评估会在同一回合输出 `ASAI_METRIC` 和 `ASAI_COMPONENTS`，包括原始/受控/平滑比值、正式及活跃战争、在产平民单位、扶持档位、当前焦点和焦点收益。总体档改变时输出 `ASAI_PACING`，焦点改变时输出 `ASAI_RECOVERY`，扶持档变化时输出 `ASAI_SUPPORT`；验证稳定后可将开关改回 `0`。
