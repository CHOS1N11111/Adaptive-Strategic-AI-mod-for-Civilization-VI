# Adaptive Strategic AI 0.5.1

这不是“给 AI 偷资源”的作弊包，而是一个可审计、可测量的 AI 工程基线。当前只支持 Gathering Storm，建议远古时代开局并新建游戏测试。0.5.1 让所有存活主要 AI 独立接受竞争护栏调节，更早响应持续落后，并根据恢复结果退出无效方向；它不设置竞争者名额，也不把 AI 锁死在固定胜利路线。
...
- 综合比值不高于 80% 时可进入强扶持档，达到 88% 时退出；两档都只增加有上限的原生 AI 决策权重，不直接赠送产出、单位或进度。
