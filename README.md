...
- Catch-up support has a modest tier and a bounded severe tier. Severe support can activate when overall strength collapses, at least two core pillars fail together, or one pillar falls critically low. On Deity only, confirmed severe support activates a capped `+40%` Production and `+30%` Science/Culture city-yield result layer; the exact inverse is applied after recovery. Only one pillar recovery runs at a time; an ineffective direction exits into its own cooldown so another valid pillar can take over.
