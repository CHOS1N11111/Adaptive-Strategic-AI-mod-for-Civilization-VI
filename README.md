...
# Adaptive Strategic AI 0.10.0

Adaptive Strategic AI reshapes Civilization VI: Gathering Storm AI behavior around expansion, economic development, military readiness, and player-relative pacing. Version 0.10.0 preserves a credible Deity opening, converts surplus armies into growth and strategic pressure, and retains bounded, reversible support for AI civilizations that fall behind.
...
- Scale recovery also runs independently from the shared pillar focus. From Standard-equivalent turn 50 it enters after two evaluations at or below 75% of the player's smoothed empire scale, or immediately when the raw empire ratio reaches 60%. It remains active across science, culture, and empire focus changes until the smoothed empire ratio reaches 88%. While active, it favors population, production, improvements, trade capacity, Aqueducts, Neighborhoods, Industrial Zones, Commercial Hubs/Harbors, and foundational buildings while increasing wonder opportunity cost.
- Scale recovery permits one additional Settler pipeline, capped at three total. Active major wars still disable expansion recovery. Military readiness pauses extra expansion below 2.25 combat units per planned city unless the AI is already in military dominance, preventing a thin army from cancelling the defensive floor without allowing an overwhelmingly stronger army to deadlock growth.
...
`ASAI_ENABLE_METRICS` in `AI/10_CoreEconomy.sql` defaults to `1`. Each controller evaluation emits `ASAI_METRIC`, `ASAI_COMPONENTS`, `ASAI_ECONOMY`, `ASAI_CONVERSION`, and `ASAI_MILITARY`. These entries cover relative strength, the weakest core pillar, economic conversion, scale recovery, military dominance, the active Settler cap, city defenses, unit roles, military density, the current military queue target, and whether execution recovery is active. Focus reviews emit `ASAI_FOCUS`. State changes use `ASAI_PACING`, `ASAI_RECOVERY`, `ASAI_SUPPORT`, `ASAI_RESULT`, `ASAI_SCALE`, `ASAI_READINESS`, and `ASAI_DOMINANCE`, including reason fields where applicable.
