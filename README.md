...
# Adaptive Strategic AI 0.11.0

Adaptive Strategic AI reshapes Civilization VI: Gathering Storm AI behavior around expansion, economic development, military readiness, and competitive pacing. Version 0.11.0 adds a rolling strategic coordinator so the AI commits its main production pressure to one coherent plan, keeps one compatible support focus, and changes direction as the game changes.
...
`ASAI_ENABLE_METRICS` in `AI/10_CoreEconomy.sql` defaults to `1`. Each controller evaluation emits `ASAI_METRIC`, `ASAI_COMPONENTS`, `ASAI_STRATEGY_INPUT`, `ASAI_ECONOMY`, `ASAI_CONVERSION`, and `ASAI_MILITARY`. Plan changes and reviews emit `ASAI_PLAN` and `ASAI_PLAN_REVIEW`, including all candidate scores, support selection, execution response, gains, and stalls. Existing pacing, focus, support, result, scale, readiness, and dominance transitions remain separately attributable.
