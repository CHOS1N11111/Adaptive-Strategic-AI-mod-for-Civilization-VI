...
这不是“给 AI 偷资源”的作弊包，而是一个可审计、可测量的 AI 工程基线。当前只支持 Gathering Storm，建议远古时代开局并新建游戏测试。0.8.2 根据一局联机速度第 1-76 回合完整复盘加入战前军事戒备；该局的朝鲜和祖鲁都在防线已经崩塌后才开始有效动员。
...
- 0.8.2 的生产、区域、商路、军事编成和转化数据仍不进入上述七项综合总分；只有焦点响应和独立军力护栏会临时提高对应的原生 AI 权重。

基建恢复的目标改良数为：`max(2 × 城市数, min(0.65 × 人口, 0.30 × 已拥有地块))`。已有及当前在产建造者会按每个 2 个潜在改良抵扣目标；当前在产商人同样抵扣商路缺口。商路容量低于 `ceil(城市数 / 3)` 时会偏好商业中心/港口，并给首个市场/灯塔 `+120` 的条件权重，避免容量建筑继续被其他订单挤掉。扩张恢复允许的已有及当前在产移民上限为 `max(1, ceil(城市数 / 5))`。达到上述预算后，独立反压策略降低继续生产对应平民单位的权重。
